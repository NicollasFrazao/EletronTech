...
<!doctype html>
 <html>
	...
	<div id="agenda">
		<form id="Frm_Dados" method="POST" action="../php/EditarEvento.php">
			<table>
				<tr>
				...
				<tr>
					<td colspan="2">
						<input type="hidden" id="cde" name="cdEvento"/>
				...
				<tr>
					<td>
						<input id="btnCancel" type="button" onclick="Limpar()" value="  Limpar  " >
					</td>
					<td>
						<input id="btnConfirma" type="button" value="Editar" onclick="Validar();">
					</td>
				</tr>
		...
		<script>
			...
			<?php				
				if (isset($_GET['cdEvento']))
				{
					...
					$query_Busca_Evento2 = "select * from tb_evento where cd_evento = '$cdEvento' and cd_usuario = '$codigoUsuario'";
					...
					$dtEvento = $linha_Busca_Evento2['dt_evento'];
					$dsEvento = str_replace(ConfereQuebra(), "&&quebra&&", $linha_Busca_Evento2['ds_evento']);
					
					echo "cde.value = '$cdEvento';";
					echo "evento.value = '$nmEvento';";
					echo "data.value = '$dtEvento';";
					echo "dss.innerHTML = InsereDetalhes('$dsEvento');";
				}
				else
				{
					echo "cde.value = '';";
